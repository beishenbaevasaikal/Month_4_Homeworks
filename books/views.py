...
def kids_tags_view(request):
    if request.method == "GET":
        kids_tags = models.Types.objects.filter(tags__name="Детские книги").order_by(
            "-id"
        )
        return render(
            request,
...
def fairy_tale_tags_view(request):
    if request.method == "GET":
        fairy_tale_tags = models.Types.objects.filter(tags__name="Сказки").order_by(
            "-id"
        )
        return render(
            request,
...
def love_story_tags_view(request):
    if request.method == "GET":
        love_story_tags = models.Types.objects.filter(
            tags__name="Любовная литература"
        ).order_by("-id")
        return render(
            request,
...
def all_types(request):
    if request.method == "GET":
        ...
        return render(
            request, template_name="types/all_types.html", context={"types": types}
        )
...
def hobby_view(request):
    if request.method == "GET":
        return HttpResponse(
            "В данный момент мое хобби - это изучение языков программирования"
        )
...
class SearchView(generic.ListView):
    ...
    def get_queryset(self):
        return models.Book_list.objects.filter(
